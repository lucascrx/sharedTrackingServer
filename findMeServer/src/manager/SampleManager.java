...
public enum SampleManager {
	...
	public CreationResponse createNewSession(String sessionName,String publicID,String password, Integer rate,
		...
		try {
			...
			Sanitizer.sanitizeUploadRate(rate);
			Sanitizer.sanitizeStartTimeAlone(start);
			if(end!=null){
				Sanitizer.sanitizeEndTime(end, start);
			}
			...
			this.dbAccesser.createNewGuidance(publicID, privateToken, sessionName, password,(int) rate, start,end,now);
			//generating response for client
			response = new CreationResponse(true,sessionName,rate,publicID,privateToken,start,end,now);
	 ...
	 * @return the status of the update operation and the last modification time*/
		public UpdateResponse updateSessionWithSample(String privateID, Double latitude, Double longitude, String deviceName, String deviceID){
			...
			Sanitizer.sanitizeStringAsName(deviceName);
	 ...
	 * @return the status of the update operation and the update time*/
	public UpdateResponse updateSessionStartingTime(String privateID, Timestamp start){
		UpdateResponse result = new UpdateResponse();
		java.util.Date date= new java.util.Date();
		Timestamp now = new Timestamp(date.getTime());
		try{
			...
			//sanitization
			this.dbAccesser.vefifyStartingTimeSynchro(privateID,start);
			//update
			this.dbAccesser.updateSessionStartingTime(privateID, start, now);
			result = new UpdateResponse(true,now);
	 ...
	 * @return the status of the update operation and the update time*/
	public UpdateResponse updateSessionEndingTime(String privateID, Timestamp end){
		UpdateResponse result = new UpdateResponse();
		java.util.Date date= new java.util.Date();
		Timestamp now = new Timestamp(date.getTime());
		try{
			...
			//sanitization
			this.dbAccesser.vefifyEndingTimeSynchro(privateID,end);
			//update
			this.dbAccesser.updateSessionEndingTime(privateID, end, now);
			result = new UpdateResponse(true,now);
