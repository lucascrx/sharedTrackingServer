...
public class Accesser {
	 ...
	 * @return session starting time*/
	public void vefifyStartingTimeSynchro(String guidancePrivateID, Timestamp newStart) throws SQLException, DatabaseSynchroException{
		openConnectionWithDb();	
		try{
		//resolve guidance table name and if first sample update status
			//resolving guidance table from guidance private ID 
			...
			if(rs.next()){
				...
				try{
					if(currentEndingTime==null){
						Sanitizer.sanitizeStartTimeAlone(newStart);
					}else{
						Sanitizer.sanitizeStartTime(newStart, currentEndingTime);
					}
				...
				if(!now.before(currentStartingTime)||!currentStartingTime.before(newStart)){
					//send Exception
					throw new DatabaseSynchroException(lastModifTime);
				}
			}else{
	 ...
	 * @throws SQLException, DatabaseSynchroException
	 * @return session starting time*/
	public void vefifyEndingTimeSynchro(String guidancePrivateID, Timestamp newEnd) throws SQLException, DatabaseSynchroException{
		openConnectionWithDb();	
		try{
		//resolve guidance table name and if first sample update status
			//resolving guidance table from guidance private ID 
			...
			if(rs.next()){
				...
				try{
					Sanitizer.sanitizeEndTime(newEnd, currentStartingTime);	
				} catch (SanitizationException e) {
					throw new DatabaseSynchroException(lastModifTime);
				}
				//check synchronization with current start time
