...
@Path(Constants.CONTRIBUTE_SESSION_PATH)
public class ContributionService {
		
		private final String logTag = "Jersey.Contribution : ";

		  //browser debugging
		  @GET
		  @Produces(MediaType.TEXT_HTML)
		  public String sayHtmlHello() {
		    return "<html> " + "<title>" + "Test Contribution Service" + "</title>"
		        + "<body><h1>" + "Contribution Service" + "</body></h1>" + "</html> ";
		  }
		  ...
		  public CreationResponse contribute(
			  ...
			  String header = Constants.formatHeader(logTag+"New Service for Contribution to existing Session ");
			  System.out.println(header);
			  //asking Manager for processing
			  SampleManager manager = SampleManager.INSTANCE;
			  CreationResponse response = manager.contributeToSession(publicID, password);
